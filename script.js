...
function generateProductCard(product) {
  ...
  // generate the HTML markup for the product card
  return `
...
function updateProductCounter() {
  ...
  counterElement.textContent = `${currentProductsCount} - ${totalProductsCount}`;
}
// update the product counter initially
updateProductCounter();

// function to  adding a product to the cart
...
function handleCheckboxChange() {
  ...
  selectedPrices = Array.from(priceCheckboxes)
    .filter((checkbox) => checkbox.checked && checkbox.value !== "over-40000")
    .map((checkbox) => {
  ...
  const over4000Checkbox = document.getElementById("over-40000");
  if (over4000Checkbox.checked) {
    selectedPrices.push([40000, Number.MAX_VALUE]);
  }
